...
export const fetchVideoWithApiKey = async (videoUrl: string, keys?: ApiKeys): Promise<Blob> => {
...
Provide ONLY the URL in your response. Do not include any other text, explanation, or formatting.`;
        ...
        // FIX: Replace URL.canParse with a try-catch block for broader compatibility.
        try {
            new URL(url);
            return url;
        } catch {
        ...
        throw new Error("AI research could not find a valid URL for this topic.");
    } catch (error) {
        console.error("findBestUrlForTopic failed:", error);
        const errorMessage = error instanceof Error && error.message.includes("valid URL")
            ? error.message
            : "Failed to perform web research. The API might be unavailable.";
...
- trendingTopics: An array of 3-5 recent and relevant trending topics, search queries, or news headlines related to the main topic. Use your search tool to find these.
...
Provide ONLY the raw JSON object in your response. Do not wrap it in markdown formatting or add any explanatory text.`;
        ...
        console.error("performResearch failed:", error);
        if (error instanceof Error && error.message.includes("JSON")) {
            throw new Error("The AI returned an invalid analysis format. Please try again.");
        }
        throw new Error("Failed to perform research. The API might be temporarily unavailable.");
    }
};
...
const generateAudio = async (text: string, elevenLabsKey: string): Promise<string | undefined> => {
    if (!elevenLabsKey) {
        console.warn("ElevenLabs API Key is not provided; skipping audio generation.");
        return undefined;
    }
    
    const VOICE_ID = '21m00Tcm4TlvDq8ikWAM'; // Rachel
    // FIX: Corrected typo from VOIC_ID to VOICE_ID.
    ...
    try {
        ...
        // Don't throw, just return undefined so the flow can continue without audio
        return undefined;
    }
};
...
const generateTextContent = async (platform: Platform, options: GenerationOptions, trendingTopics?: string, keys?: ApiKeys): Promise<Pick<Post, 'content' | 'groundingSources'>> => {
    try {
        ...
        throw new Error(`Failed to generate text content for ${platform}. The API may be unavailable.`);
    }
};
...
const generateImage = async (topic: string, tone: string, model: string, keys?: ApiKeys): Promise<string> => {
    try {
        ...
        throw new Error("Failed to generate image. The API may be unavailable or your prompt may have been blocked.");
    }
};
...
const generateVideo = async (
    ...
    try {
        ...
        updateCallback('Sending prompt to video model...');
        ...
        let lastMessage = 'AI is preparing your video...';
        ...
        while (!operation.done) {
            await new Promise(resolve => setTimeout(resolve, 5000)); // Poll every 5 seconds for faster updates
            try {
                ...
                if (metadata) {
                    ...
                    const progress = metadata.progressPercentage;

                    if (state && typeof state === 'string' && state !== 'STATE_UNSPECIFIED' && state !== 'SUCCEEDED') {
                        const formattedState = state.charAt(0) + state.slice(1).toLowerCase().replace(/_/g, ' ');
                        newMessage = `Status: ${formattedState}`;
                    } else {
                        newMessage = 'AI is processing your video...';
                    }

                    if (progress && typeof progress === 'number') {
                        newMessage += ` (${Math.floor(progress)}%)`;
                    }
                } else {
                    newMessage = 'AI is creating your video... This can take several minutes.';
                }
                ...
                pollingFailures++;
                console.warn(`Polling failed (${pollingFailures}/${MAX_POLLING_FAILURES}). Will retry.`, e);
                if (pollingFailures >= MAX_POLLING_FAILURES) {
                    throw new Error("Failed to get video generation status after multiple attempts. Aborting.");
                }
            }
        }

        updateCallback('Finalizing and retrieving video...');
        ...
        if (error instanceof Error && error.message.includes("entity was not found")) {
            throw new Error("Video generation failed. Your API Key may be invalid or missing required permissions. Please select a valid key.");
        }
        throw new Error("Failed to generate video. The API may be unavailable or the request could not be completed.");
    }
};
...
export const generateContentForAllPlatforms = async (
    ...
    if (useTrendingTopics) {
        ...
        try {
            ...
            // Don't throw, allow generation to continue without trends
            updateCallback('Could not fetch trends, continuing...');
        }
    }
    ...
    if (includeVideo) {
        const videoPlatforms = platforms.filter(p => 
            p === PlatformEnum.TikTok || 
            p === PlatformEnum.Instagram ||
            p === PlatformEnum.YouTube
        );

        if (videoPlatforms.length > 0) {
            const platformsByRatio = videoPlatforms.reduce((acc, p) => {
                const ratio = (p === PlatformEnum.YouTube) ? '16:9' : '9:16';
                if (!acc[ratio]) {
                    acc[ratio] = [];
                }
                acc[ratio].push(p);
            ...
            for (const ratio in platformsByRatio) {
                const currentPlatforms = platformsByRatio[ratio];
                const platformNames = currentPlatforms.join(', ');

                updateCallback(`Generating ${ratio} video for ${platformNames}...`);
                
                const postsToUpdate = posts.filter(p => currentPlatforms.includes(p.platform));
                postsToUpdate.forEach(p => {
                    postUpdateCallback(p.id, { videoUrl: 'GENERATING' });
                });

                generateVideo(options, ratio as '16:9' | '9:16', updateCallback, apiKeys)
                    .then(finalVideoUrl => {
                        postsToUpdate.forEach(p => {
                    ...
                    .catch(err => {
                        console.error(`Video generation failed for ${ratio} ratio`, err);
                        errorCallback(err as Error);
                        postsToUpdate.forEach(p => {
                            postUpdateCallback(p.id, { videoUrl: 'FAILED' });
                        });
                    });
            }
        }
